...
## Core Gameplay Loop
1. Start a run with a partially neutral grid, starting currency, and inactive hardpoints.
2. Enemies and corruption pressure begin immediately and escalate over time.
3. Spend coins to activate hardpoints, build towers, and buy upgrades.
...
2. Each hardpoint begins the run inactive and empty.
3. The player spends coins to activate a hardpoint.
4. Once active, that hardpoint can host one permanent tower archetype.
5. If a tower is destroyed, the hardpoint remains active but becomes empty until rebuilt.

#### Rules
- Towers can only be built on active hardpoints.
- One hardpoint can hold only one permanent tower at a time.
- A hardpoint cannot be activated twice.
...
- If inactive and the player has enough coins, the activation command becomes available.
- If active and empty, the build menu for available tower archetypes becomes available.
- If active and occupied, the sidebar shows that tower's shared upgrade paths and status readout.

#### Boundaries and Edge Cases
- If the player cannot afford activation, the hardpoint remains selectable but the action is disabled.
...
#### State Machine / Flow
- States: `inactive`, `active_empty`, `active_occupied`, `destroyed_waiting_rebuild`
- Entry triggers: run start, activation purchase, tower build, tower destruction
- Exit triggers: activation, build, destruction, rebuild

#### UI / UX Requirements
- Inactive hardpoints should be visible but visually subdued.
- Active empty hardpoints should read as available infrastructure.
...
#### Implementation Notes for Coder
- Hardpoint state should be independent from tower state so rebuilding does not require reactivation.
...
#### Core Mechanics
1. At build time, the player chooses one base tower archetype for the selected active hardpoint.
...
- On each target acquisition, evaluate valid candidates against the weighted biases.
- Choose a target according to the resulting weighted roll.
...
- If an orb reaches a dead end, it either turns around or expires based on archetype rules.
...
- Update total corruption percentage after all spread and cleaning events in the interval are resolved.
...
- Higher intensity red must look meaningfully worse than low intensity red.
...
2. Coins are spent on hardpoint activation, tower construction, and global upgrades for each tower archetype.
...
- Deduct activation/build/upgrade costs immediately on successful purchase.
...
- Resolve `Tower Striker` pathing by selecting a tower target and routing with semi-random movement that still converges on that target.
...
#### State Machine / Flow
- States: `idle_display`, `selection_display`, `warning_display`, `surge_display`
- Entry triggers: selection change, threshold proximity, surge start
- Exit triggers: deselection, risk reduction, surge end

#### UI / UX Requirements
- Survival-critical information takes priority over flavor information.
- Warnings should escalate visually as corruption approaches the loss threshold.
- Readability takes precedence over strict minimalism.
...
- Early game should let the player activate at least two hardpoints and build at least one tower quickly.
...
- Right sidebar: selected hardpoint/tower details, build choices, upgrades, behavior swap, power tower status.
